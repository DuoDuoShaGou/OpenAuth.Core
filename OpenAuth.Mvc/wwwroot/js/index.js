...
layui.config({
 ...
 //               '<p>郑重提示：OpenAuth.Core 2.0新版上线，如以前访问过本站点请清空缓存后访问</p>' +
	...
	if(window.sessionStorage.getItem("menu") != null){
		...
		for(var i=0;i<menu.length;i++){
			...
			if(curmenu != "undefined"){
				if(curmenu == '' || curmenu == "null"){  //定位到Home page
	...
	$(".closePageOther").on("click",function(){
		if($("#top_tabs li").length>2 && $("#top_tabs li.layui-this cite").text()!="Home page"){
		...
		}else if($("#top_tabs li.layui-this cite").text()=="Home page" && $("#top_tabs li").length>1){
			$("#top_tabs li").each(function(){
				if($(this).attr("lay-id") != '' && !$(this).hasClass("layui-this")){
					element.tabDelete("bodyTab",$(this).attr("lay-id")).init();
					window.sessionStorage.removeItem("menu");
...
function loading(bool, text) {
		...
		if (!!text) {
			...
			$loadingtext.html("文件上传中，请稍后…");
	}
	
	$loadingtext.css("left", ($(".layui-body").first().width() + $(".layui-side.layui-bg-black").width() - $loadingtext.width()) / 2 );
	$loadingtext.css("top", ($(".layui-body").first().height() - $loadingtext.height()) / 2);
	}
